...
var url =
  "https://raw.githubusercontent.com/tlfrd/pay-ratios/master/data/payratio.json";
...
d3.json(url, function(error, data) {
  ...
  var leftSlopeCircle = slopeGroups
    .append("circle")
    .attr("r", config.radius)
    .attr("cy", d => y1(d.values[0].max / d.values[0].min));

  var leftSlopeLabels = slopeGroups
    .append("g")
  ...
  var rightSlopeCircle = slopeGroups
    .append("circle")
    .attr("r", config.radius)
    .attr("cx", config.width)
    .attr("cy", d => y1(d.values[1].max / d.values[1].min));

  var rightSlopeLabels = slopeGroups
    .append("g")
  ...
  var titles = svg.append("g").attr("class", "title");
  ...
  leftSlopeLabels.selectAll("text").attr("y", d => d.yLeftPosition);

  relax(rightSlopeLabels, "yRightPosition");
  rightSlopeLabels.selectAll("text").attr("y", d => d.yRightPosition);

  d3.selectAll(".slope-group").attr("opacity", config.unfocusOpacity);
